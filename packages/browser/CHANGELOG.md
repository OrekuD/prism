...
- Normalized `endpoint` in `createBrowserClient`: trailing `/api/v2/ingest`, slash, query, and hash are stripped with a console warning. Prevents the setup-tab origin+path doubling.
...
- Initial public release. Browser client with history/manual page views, error reporter, offline queue, and consent gate.
