...
describe("createBrowserClient", () => {
	...
	it("delivers through the runtime endpoint with core-owned auth headers", async () => {
		...
		expect(bodies[0]?.url).toBe(
			"https://analytics.self-hosted.example/api/v2/ingest",
		);
	...
	it("persists the queue through localStorage (reload persistence wiring)", async () => {
		...
		const snapshot = JSON.parse(
			window.sessionStorage.getItem(keys[0] ?? "") ?? "{}",
		) as {
			events: unknown[];
		};
		...
		expect(
			JSON.parse(window.sessionStorage.getItem(keysAfter[0] ?? "") ?? "{}")
				.events,
		).toHaveLength(0);
...
describe("runtime edge cases (branch coverage)", () => {
	...
	it("fires background lifecycle events on hidden visibility", async () => {
		const events: string[] = [];
		const runtime = createBrowserRuntime();
		const remove = runtime.lifecycle?.on("background", () =>
...
describe("release review — timeouts, keepalive budget, multi-tab safety", () => {
	...
	it("namespaces each tab's queue so offline persistence never collides", async () => {
		...
		const tabA = await createBrowserClient({
			...BASE,
			queue: { maxRetries: 5 },
		});
		tabA.track("from_a");
		...
		const tabB = await createBrowserClient({
			...BASE,
			queue: { maxRetries: 5 },
		});
		tabB.track("from_b");
...
describe("identity + global properties in the browser (task-10 §7)", () => {
	...
	it("persists the persistent global-property scope through local storage", async () => {
		...
		expect(
			JSON.parse(window.localStorage.getItem(persistentKeys[0] ?? "") ?? "{}")
				.referral,
		).toBe("friend");
		...
		expect(
			JSON.parse(window.sessionStorage.getItem(sessionKeys[0] ?? "") ?? "{}")
				.tab_pick,
		).toBe("left");
...
describe("shared-device reset/reload (task-10 review F1)", () => {
	...
	it("a reload after logout never adopts the previous user's identity", async () => {
		...
		const anonymousEvent = newEnvelope.events.find(
			(e) => e.name === "anonymous_after_reset",
		);
