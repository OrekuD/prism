...
describe("packaged consumption", () => {
	it("packs, installs, and runs against an explicit self-hosted endpoint", () => {
		...
		try {
			...
			const coreTarball = (
				JSON.parse(corePackJson) as Array<{ filename: string }>
			)[0];
			...
			if (packed.length === 0 || !coreTarball)
			...
			return installed
				...
				.then(async (prism) => {
					...
					expect(String(url)).toBe(
						"https://self-hosted.prism.example/api/v2/ingest",
					);
