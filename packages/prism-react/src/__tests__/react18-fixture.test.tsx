...
describe("React 18 peer compatibility", () => {
  it(
    "renders the provider under react@18 from a clean install",
    async () => {
      ...
      try {
        ...
        const coreTarball = (JSON.parse(
          execSync("npm pack --json " + cache + " --pack-destination " + packDir, {
            cwd: CORE_DIR,
            encoding: "utf8",
          }),
        ) as Array<{ filename: string }>)[0];
        const reactTarball = (JSON.parse(
          execSync("npm pack --json " + cache + " --pack-destination " + packDir, {
            cwd: PKG_DIR,
            encoding: "utf8",
          }),
        ...
        expect(document.getElementById("react18-child")?.textContent).toBe("rendered");
        root.unmount();
