...
describe("queue and delivery", () => {
	...
	it("drops events beyond the queue byte capacity", async () => {
		...
		expect(prism.track("a", { payload: "x".repeat(200) }).status).toBe(
			"queued",
		);
...
describe("sessions", () => {
	it("attaches the session ID to events while a session is active", async () => {
		...
		await prism.flush();
		const batch = deliveredEvents(bodies[0]) as Array<{
			sessionId?: string;
			name: string;
		}>;
...
describe("anonymous identity persistence", () => {
	...
	it("rejects persistent identity without durable storage", async () => {
		const runtime = fakeRuntime() as PrismRuntimeAdapter & {
			storage?: PrismRuntimeAdapter["storage"];
		};
...
describe("property sanitization (review fix)", () => {
	it("redacts credential keys at any depth and in arrays", async () => {
		...
		await prism.flush();
		const delivered = deliveredEvents(bodies[0]) as Array<{
		...
		expect(
			(
				(props.user as Record<string, unknown>).nested as Record<
					string,
					unknown
				>
			).apiKey,
		).toBe("[REDACTED]");
		expect((props.tags as Array<Record<string, unknown>>)[0]?.token).toBe(
			"[REDACTED]",
		);
	...
	it("honors a custom deny list", async () => {
		...
		const result = prism.track("hr_event", {
			employee_email: "a@b.c",
			name: "Ada",
		});
		expect(result.status).toBe("queued");
	});
	...
	it("throws when a property string exceeds the length limit", async () => {
		...
		expect(() => prism.track("long", { note: "x".repeat(11) })).toThrow(
			/length/i,
		);
	});
...
describe("oversized single event (review fix)", () => {
	it("delivers an event larger than maxBatchBytes as a solo batch", async () => {
		...
		expect(prism.track("big", { payload: "x".repeat(400) }).status).toBe(
			"queued",
		);
		await prism.flush();
		expect(bodies).toHaveLength(1);
...
describe("strict JSON + dangerous keys (review fix)", () => {
	it("rejects prototype-pollution keys", async () => {
		...
		expect(() => prism.track("polluted", polluted as never)).toThrow(
			/dangerous-key/,
		);
		expect(() =>
	...
	it("rejects runtime-only non-JSON values", async () => {
		const prism = await ready();
		expect(() => prism.track("fn", { cb: () => 1 } as never)).toThrow(
			/JSON values/,
		);
		expect(() => prism.track("undef", { missing: undefined } as never)).toThrow(
			/JSON values/,
		);
		expect(() => prism.track("big", { n: BigInt("10") } as never)).toThrow(
			/JSON values/,
		);
	});
});
...
describe("queue persistence (slice 3)", () => {
	it("persists and restores queued events through storage with a versioned key", async () => {
		...
		expect(
			(JSON.parse(stored.get(key ?? "") ?? "{}") as { events: unknown[] })
				.events,
		).toEqual([]);
...
function schedulableRuntime() {
	...
	return {
		...
		async fire(
			predicate: (entry: { delayMs: number }) => boolean,
		): Promise<void> {
...
describe("strict per-event reconciliation (slice 3 corrections)", () => {
	it("keeps the batch when results reference unrelated ids", async () => {
		...
		runtime.transport.post = async () => ({
			...
			text: async () =>
				JSON.stringify({
					results: [{ id: "unrelated-9", status: "accepted" }],
				}),
		});
		const prism = await ready({
			runtime,
			queue: { maxRetries: 5, flushIntervalMs: 10_000_000 },
		});
		prism.track("x1");
		prism.track("x2");
		prism.track("x3");
	...
	it("removes only submitted events with terminal results", async () => {
		...
		const firstId =
			events[0]?.status === "queued" ? events[0].eventId : "missing";
		...
		mode = "good";
		await prism.flush();
		const last = deliveredEvents(bodies[bodies.length - 1]) as Array<{
			name: string;
		}>;
...
describe("bounded scheduled retries (slice 3 corrections)", () => {
	...
	it("drops the batch after maxRetries and stops retrying", async () => {
		...
		const prism = await ready({
			runtime,
			queue: { maxRetries: 2, flushIntervalMs: 10_000_000 },
		});
		const codes: string[] = [];
		prism.onDiagnostic((d) => codes.push(d.code));
		prism.track("doomed");
		...
		const pending = scheduled.filter(
			(e) => e.delayMs < 10_000_000 && !e.cancelled,
		);
...
describe("transport authentication (review F1)", () => {
	...
	it("never leaks the project key into diagnostics or error messages", async () => {
		...
		const all =
			JSON.stringify(codes) + JSON.stringify(codes.map((c) => c.message));
...
describe("anonymous identity (review F2)", () => {
	...
	it("attaches an existing persisted ID to every delivered event", async () => {
		const stored = new Map<string, string>([
			["prism:anonymous_id", "stored-id-123"],
		]);
...
describe("wire ceilings (review F3)", () => {
	it("rejects queue settings above the wire ceilings with a specific error", async () => {
		await expect(ready({ queue: { maxBatchEvents: 51 } })).rejects.toThrow(
			/maxBatchEvents 51 exceeds the wire ceiling \(50\)/,
		);
		await expect(
	...
	it("rejects runtime-only property values before sanitization", async () => {
		...
		expect(bad({ many: Array.from({ length: 101 }, () => 1) })).toThrow(
			/too-many-elements/,
		);
...
describe("retry-after minimum (review F9)", () => {
	it("never schedules before a delta-seconds Retry-After, even above 60s", async () => {
		...
		const prism = await ready({
			runtime,
			queue: { maxRetries: 3, flushIntervalMs: 10_000_000 },
		});
		prism.track("slow");
		await expect(prism.flush()).rejects.toThrow(/429/);
...
describe("consent race with abort-ignoring transports (harden F15)", () => {
	it("never requeues or delivers again when consent is denied mid-flight", async () => {
			...
			return {
				...
				text: async () =>
					JSON.stringify({
						results: [
							{
								id: (JSON.parse(body).events[0] as { eventId: string }).eventId,
								status: "unknown",
							},
						],
					}),
		...
		const prism = await ready({
			runtime,
			queue: { maxBatchEvents: 1, flushIntervalMs: 10_000_000 },
		});
...
describe("wire context allowlist (harden F16)", () => {
	it("picks only known fields and redacts credentials in context", async () => {
		...
		const context = (runtime as unknown as { context: Record<string, unknown> })
			.context;
	...
	it("rejects a runtime context that is not JSON-safe at factory creation", async () => {
		...
		await expect(ready({ runtime })).rejects.toThrow(
			/runtime context is not JSON-safe/,
		);
	});
});
...
describe("session properties validation (harden F19)", () => {
	it("rejects non-JSON session properties instead of silently converting", async () => {
		...
		expect(badSession({ score: Number.POSITIVE_INFINITY })).toThrow(
			/non-finite-number/,
		);
...
describe("multi-context persistence (release review)", () => {
	it("never erases another context's queued events (owner-segmented merge)", async () => {
		...
		expect(snapshot.events.map((e) => e.name).sort()).toEqual([
			"from_a",
			"from_b",
		]);
	});
