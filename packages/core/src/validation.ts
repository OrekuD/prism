...
export function sanitizeProperties(
	...
	const maxStringLength =
		options.maxStringLength ?? INGEST_LIMITS.maxStringLength;
	...
	const walk = (value: unknown, depth: number): JsonValue => {
		...
		if (typeof value === "string") {
			if (value.length > maxStringLength) {
				throw new Error(
					`Event property string exceeds maximum length (${maxStringLength})`,
				);
			}
			return value;
		}
		...
		if (value !== null && typeof value === "object") {
			...
			for (const [key, entry] of Object.entries(value)) {
				if (DANGEROUS_KEYS.has(key)) {
					throw new Error(
						`Event properties contain a dangerous key ("${key}")`,
					);
				}
...
export function validateJsonValue(
	...
	const maxArrayElements =
		options.maxArrayElements ?? INGEST_LIMITS.maxArrayElements;
...
export function assertJsonSerializable(
	...
	if (!result.ok) {
		throw new Error(
			`Event properties must contain only JSON values (${result.reason})`,
		);
	}
}
