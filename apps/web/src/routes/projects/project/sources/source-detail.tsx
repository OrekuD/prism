...
export function SourceDetail() {
	...
	// Per-source error collection config (task-15 item 440) — local draft
	// initialized from the server row, persisted explicitly with Save.
	...
	const [errorDraft, setErrorDraft] = React.useState<SourceErrorSettings | null>(null);
	const errorDirty =
		errorDraft !== null &&
		...
		<div className="flex flex-1 flex-col space-y-6">
			...
			<Frame className="p-6">
				...
				<p className="mt-1.5 text-[13px] text-text-muted">
					...
					{data.platform === "server" ? "secret" : "publishable"} key is shown
					once at creation.
				</p>
				<div className="mt-4">
					{snippetKey ? (
			...
			{data.platform === "web" && canManage ? (
				<Frame className="p-6">
					...
					<p className="mt-1.5 text-[13px] text-text-muted">
						...
						key. Adding or removing one saves immediately.
					</p>
			...
			<Frame className="p-6">
				<div className="flex items-start justify-between gap-4">
					<div className="space-y-1.5">
						<SectionLabel>Error collection</SectionLabel>
						<p className="text-[13px] text-text-muted">
					...
					{errorSettings.isLoading ? (
						...
						<Badge variant={errorSettings.data?.mode === "off" ? "outline" : "default"}>
							{errorSettings.data
				...
				{errorSettings.isError ? (
					...
					<div className="mt-4 space-y-5">
						...
						<div className="flex flex-wrap items-center gap-x-5 gap-y-1 font-mono text-[12px] text-text-muted">
							<span>
								{errorSettings.data?.errorCount30d ?? 0} errors in the last 30 days
							</span>
							<span>
									...
									<div className="space-y-1.5">
										...
										<Select
											...
											onValueChange={(value) =>
												setErrorDraft((prev) =>
													prev
														? { ...prev, mode: value as SourceErrorMode }
														: prev,
												)
											}
										>
										...
										<p className="text-[11.5px] leading-[1.45] text-text-subtle">
											"Global handlers" installs window error + rejection
											listeners in the SDK — an explicit privacy opt-in.
										</p>
									</div>
									<div className="space-y-1.5">
										...
										<p className="text-[11.5px] leading-[1.45] text-text-subtle">
											Sampling is applied client-side; nothing is ever sent
											beyond the sample you choose.
										</p>
									</div>
								</div>
