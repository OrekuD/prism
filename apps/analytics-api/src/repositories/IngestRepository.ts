...
export class IngestRepository {
	...
	async persistBatch(
		...
		source?: { sourceId: string; platform: string },
		// Task 17 slice 4: server-built page-view projections keyed by batch
		// index. Inserted INSIDE this transaction ONLY for events whose insert
		// won idempotency - duplicates/rejections never advance page metrics.
		...
		try {
				...
				if (claim.rowsAffected === 1) {
                  ...
                  ON CONFLICT (project_id, user_id) DO NOTHING`,
					...
					const resolvedPerson = String(
						(authoritative.rows[0] as { person_id?: unknown } | undefined)
							?.person_id ?? "",
					);
			...
			if (missingAnon.length > 0 || missingUsers.length > 0) {
				...
				for (const row of anonRows.rows) {
					const anonId = String(
						(row as { anonymous_id?: unknown }).anonymous_id ?? "",
					);
					if (anonId && !effectiveAnonymous.has(anonId)) {
						effectiveAnonymous.set(
							anonId,
			...
			const projectionByIndex = new Map(
				pageProjections.map((entry) => [entry.index, entry.row]),
			);
			for (let index = 0; index < events.length; index += 1) {
				...
				const projection = projectionByIndex.get(index);
				if (projection) {
					await tx.execute({
						sql: INSERT_PAGE_VIEW_SQL,
						args: [
							projectId,
							event.eventId,
							...
							projection.path,
							projection.title,
							projection.navigationType,
							projection.pageSequence,
							projection.previousPath,
							projection.referrerHost,
							projection.campaignSource,
							projection.campaignMedium,
							projection.campaignName,
							projection.browserFamily,
							projection.browserMajor,
							projection.osFamily,
							projection.osMajor,
							projection.deviceType,
							projection.isBot ? 1 : 0,
							projection.uaParserVersion,
							projection.viewportWidth,
							projection.viewportHeight,
							projection.primaryLanguage,
							projection.countryCode,
							projection.region,
							projection.city,
							projection.geoProvider,
						],
					});
				}
			}
...
/** The server-built projection row (Task 17 section 6): no raw UA/IP fields. */
export interface PageViewProjectionRow {
	occurredAt: number;
	host: string;
	path: string;
	title: string | null;
...
const INSERT_PAGE_VIEW_SQL = [
	"INSERT INTO web_page_views (",
	"  project_id, event_id, occurred_at, host, path, title,",
	"  navigation_type, page_sequence, previous_path,",
	"  referrer_host, campaign_source, campaign_medium, campaign_name,",
	"  browser_family, browser_major, os_family, os_major,",
	"  device_type, is_bot, ua_parser_version,",
	"  viewport_width, viewport_height, primary_language,",
