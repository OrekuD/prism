...
export default class MailManager {
	...
	public static async dispatch(ctx: Context<HonoConfig>, mail: MailProps, recipientEmail: string) {
		if (!this.resend) {
		...
		switch (mail.name) {
			...
			case 'reset-password':
				subject = 'Password Reset Request for Your Bazaar Account';
				html = generateResetPasswordTemplate(mail.props);
				break;

			case 'confirm-email':
				subject = 'Confirm Email for your Bazaar Account';
			...
			case 'otp-sign-in':
				subject = 'Your login code for Bazaar';
				html = generateOTPSignInTemplate(mail.props);
				break;

			case 'new-email':
				subject = 'Verify Your New Email Address for Your Bazaar Account';
