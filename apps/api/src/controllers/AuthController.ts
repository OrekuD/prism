...
export class AuthController {
  public static async signIn(ctx: Context<HonoConfig>) {
    ...
    if (
      ...
      return ctx.json(
        new ErrorResponse("max_number_of_failed_login_attempts"),
        400,
      );
    }
  ...
  public static async magicLinkSignIn(ctx: Context<HonoConfig>) {
    ...
    const data = validateData<MagicLinkSignInRequest>(
      MagicLinkSignInRequestSchema,
      body,
    );
  ...
  public static async requestOTPSignIn(ctx: Context<HonoConfig>) {
    ...
    const user =
      (await db`SELECT id FROM users WHERE email = ${data.email.toLowerCase().trim()}`) as Array<User>;
  ...
  public static async signUp(ctx: Context<HonoConfig>) {
    ...
    const users =
      (await db`SELECT id FROM users WHERE email = ${data.email.trim().toLowerCase()}`) as Array<User>;
    ...
    let user =
      (await db`INSERT INTO users (email, password, role) VALUES (${data.email.trim().toLowerCase()}, ${hashedPassword}, ${
        Roles.USER
    ...
    const verifyEmailLink = await AuthController._generateVerifyEmailURL(
      ctx,
      user[0].id,
    );
  ...
  public static async verifyEmail(ctx: Context<HonoConfig>) {
    ...
    const data = validateData<VerifyEmailRequest>(
      VerifyEmailRequestSchema,
      body,
    );
    ...
    const user =
      await db`SELECT id FROM users WHERE id = ${decoded.payload.userId}`;
  ...
  public static async resetPassword(ctx: Context<HonoConfig>) {
    ...
    const data = validateData<ResetPasswordRequest>(
      ResetPasswordRequestSchema,
      body,
    );
  ...
  public static async forgotPassword(ctx: Context<HonoConfig>) {
    ...
    const data = validateData<ForgotPasswordRequest>(
      ForgotPasswordRequestSchema,
      body,
    );
    ...
    const resetLink = await AuthController._generateResetPasswordURL(
      ctx,
      user[0].id,
    );
  ...
  private static async _failedLoginAttempt(
    ctx: Context<HonoConfig>,
    userId: string,
  ) {
    const db = DatabaseManager.getInstance(ctx);
    const loginAttempts =
      (await db`SELECT number_of_attempts FROM login_attempts WHERE user_id = ${userId}`) as Array<LoginAttempt>;
  ...
  public static async _generateVerifyEmailURL(
    ctx: Context<HonoConfig>,
    userId: string,
  ) {
  ...
  public static async _generateMagicLink(
    ctx: Context<HonoConfig>,
    userId: string,
  ) {
    const accessToken = crypto.randomBytes(128).toString("hex");
