...
import ChangePasswordRequest, { ChangePasswordRequestSchema } from '../network/requests/ChangePasswordRequest';
import ErrorResponse from '../network/responses/ErrorResponse';
import OkResponse from '../network/responses/OkResponse';
import ChangeEmailRequest, { ChangeEmailRequestSchema } from '../network/requests/ChangeEmailRequest';
import Profile from '../models/Profile';
import UpdateUserRequest, { UpdateUserRequestSchema } from '../network/requests/UpdateUserRequest';
...
export default class UserController {
	...
	public static async changePassword(ctx: Context<HonoConfig>) {
		...
		if (!bcrypt.compareSync(data.oldPassword, user!.password)) {
			return ctx.json(new ErrorResponse('old_password_incorrect').toJSON(), 400);
		}

		const hashedPassword = bcrypt.hashSync(data.newPassword, bcrypt.genSaltSync());

		const db = DatabaseManager.getInstance(ctx);

		const rows = await db`UPDATE users SET password = ${hashedPassword} WHERE id = ${user!.id} RETURNING id`;
	...
	public static async changeEmail(ctx: Context<HonoConfig>) {
		...
		const profile = (await db`SELECT email_verified_at FROM profiles WHERE user_id = ${userObject!.id}`) as Array<Profile>;
	...
	public static async sendVerifyEmail(ctx: Context<HonoConfig>) {
		const user = ctx.get('user');
		const confirmEmailLink = await AuthController._generateVerifyEmailURL(ctx, user!.id);
	...
	public static async updateProfilePicture(ctx: Context<HonoConfig>) {
		...
		const uploadResult = await UploadController.uploadSingle(ctx, file, '/users/profile-pictures', `${file.name}`);
		...
		const profilePicture = (await db`SELECT profile_picture_id FROM profile_pictures WHERE user_id = ${user.id}`) as Array<ProfilePicture>;

		if (profilePicture.length > 0) {
			await UploadController.deleteFile(ctx, profilePicture[0].profile_picture_id);
	...
	public static async updateUser(ctx: Context<HonoConfig>) {
		...
		let updatedUser = (await db`UPDATE users SET user_name = ${data.userName} WHERE id = ${user!.id} RETURNING id`) as Array<User>;

		const updatedProfile = await db`UPDATE profiles SET first_name = ${data.firstName}, last_name = ${data.lastName}, gender = ${
			data.gender
